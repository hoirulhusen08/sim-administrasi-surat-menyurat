...
<div class="container-fluid">
    ...
    <!-- Datatables -->
    <h5><a style="text-decoration: none;" href="<?= base_url('lead/outgoingMail'); ?>"><i class="bi bi-arrow-left-circle-fill"></i> <small>Kembali</small></a></h5>
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="alert alert-warning" role="alert">
                    ...
                    <form method="post" action="<?= base_url('lead/viewOutgoingMail?id=') . $lihat_surat_keluar['id']; ?>">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="status"><span style="color:red;">*</span> Status Surat <small>(Wajib diisi)</small></label>
                                    <select name="status" id="status" class="form-control <?= (form_error('status') ? 'is-invalid' : '') ?>" <?= ($lihat_surat_keluar['status'] >= 3) ? 'disabled' : ''; ?>>
                                        <option value="">-- Pilih Surat --</option>
                                        <option value="0" hidden>Belum Dicek</option>
                                        <option value="1">Kembalikan (Revisi)</option>
                                        <option value="2">Teruskan (Proses Dilanjutkan)</option>
                                        <option value="3" hidden>Teruskan (Minta TTD)</option>
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    <label for="tindak_lanjut"><span style="color:red;">*</span> Tindakan Lanjut <small>(Wajib diisi)</small></label>
                                    <textarea rows="4" name="tindak_lanjut" class="form-control <?= (form_error('tindak_lanjut') ? 'is-invalid' : '') ?>" id="tindak_lanjut" placeholder="Tindak lanjut yang harus dilakukan..." <?= ($lihat_surat_keluar['status'] >= 3) ? 'disabled' : ''; ?>><?= set_value('tindak_lanjut'); ?><?= $lihat_surat_keluar['tindak_lanjut']; ?></textarea>
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <textarea rows="4" name="catatan" class="form-control <?= (form_error('catatan') ? 'is-invalid' : '') ?>" id="catatan" placeholder="Berikan catatan jika ada..." <?= ($lihat_surat_keluar['status'] >= 3) ? 'disabled' : ''; ?>><?= set_value('catatan'); ?><?= $lihat_surat_keluar['catatan']; ?></textarea>
                                    <?= form_error('catatan', '<small class="text-danger pl-1">', '</small>'); ?>
                                </div>
                            </div>
                        </div>

                        <button type="submit" class="btn btn-primary" <?= ($lihat_surat_keluar['status'] >= 3) ? 'disabled' : ''; ?>><i class="bi bi-send"></i> Kirim Validasi</button>
                    </form>
                </div>
            </div>
        </div>
