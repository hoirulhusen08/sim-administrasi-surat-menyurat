...
<div class="container-fluid">
    ...
    <!-- Datatables -->
    <h5><a style="text-decoration: none;" href="<?= base_url('dekan/outgoingMail'); ?>"><i class="bi bi-arrow-left-circle-fill"></i> <small>Kembali</small></a></h5>
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="alert alert-warning" role="alert">
                    <h5 class="text-uppercase font-weight-bold mb-4 mt-2">Beri Izin Tanda Tangan Disini :</h5>
                    <hr class="border-dark">
                    <form method="post" action="<?= base_url('dekan/viewOutgoingMail?id=') . $lihat_surat_keluar['id']; ?>">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" class="form-control <?= (form_error('status') ? 'is-invalid' : '') ?>" <?= ($lihat_surat_keluar['status'] >= 4) ? 'disabled' : ''; ?>>
                                        ...
                                        <option value="1" hidden>Kembalikan (Revisi)</option>
                                        <option value="2" hidden>Teruskan (Proses Dilanjutkan)</option>
                                        <option value="3" hidden>Teruskan (Minta TTD)</option>
                                        <option value="4">Setujui (ACC TTD)</option>
                                    </select>
                                    <?= form_error('status', '<small class="text-danger pl-1">', '</small>'); ?>
                                </div>
                                <button type="submit" class="btn btn-primary" <?= ($lihat_surat_keluar['status'] >= 4) ? 'disabled' : ''; ?>><i class="bi bi-send"></i> Kirim</button>
                                <p><small><i class="bi bi-info-circle-fill"></i> <strong>Catatan Penting :</strong> Langkah ini hanya dapat dilakukan sekali, lakukan dengan teliti dan cermat!</small></p>
                            </div>
                            <div class="col-lg-6">
